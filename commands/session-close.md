---
description: "Alias for `/session close` — the session's Goal is done: write its Outcome, run the tool's `session close` (which gates first), mark the plan part done, post the closing note. Typed only; never invoked by Claude on its own."
