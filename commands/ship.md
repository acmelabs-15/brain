...
Spawn three subagents concurrently with the Agent tool, passing each persona's name as `subagent_type` (they are installed in `~/.claude/agents/`). **Issue all three Agent calls in a single assistant turn so they run in parallel** — sequential calls defeat the purpose of this command.
...
If the Agent tool is unavailable, read each persona file in `~/.claude/agents/` and apply it sequentially in the main context, treating the outputs as if returned in parallel — the merge phase still works.
...
- For richer multi-agent collaboration where teammates talk to each other instead of just reporting back, see `~/.claude/references/orchestration-patterns.md`.

**Persona resolution.** A project-level `.claude/agents/<name>.md` takes precedence over the user-level copies in `~/.claude/agents/`, so a repo can override any persona and `/ship` picks that up automatically.
