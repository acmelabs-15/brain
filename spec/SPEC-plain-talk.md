...
| Codex CLI | `AGENTS.md` in the user's repo | `plain-talk/AGENTS-block.md`, a block the `setup` skill writes | the user, once per repo, through `setup` |
| Antigravity CLI | `AGENTS.md` in the workspace | the same block | the user, once per repo, through `setup` |
...
`bun run plain-talk:render` writes three files from the source text and three
small templates, and `-- --check` exits 1 if any written file differs from what it
would write now. The generated files are committed, so an install needs no build.
...
| `plain-talk/AGENTS-block.md` | a `## Talk plain` heading and an HTML comment marking the block's start and end, so `setup` can replace it in place |

The body is byte-identical in all three outputs. That is a test.
...
```
bun run plain-talk:render            # write the three outputs
...
`bun:test` in `scripts/plain-talk/__tests__/render.test.ts`, against a fixture
source and fixture templates in a temp directory: three files written; the body is
identical in all three; the output-style frontmatter carries the four fields;
`--check` passes after a render and fails after a one-byte edit of an output; a
template without `{{body}}` is an error.
...
- **Never:** put a host-specific tool name in the body; hand-edit an output.
...
4. After `setup` runs in a repo, the repo's `AGENTS.md` holds the block between the markers, and a second run changes nothing. Verified in the `setup` module.
5. The body of the three outputs is byte-identical, by test.
...
2. Antigravity can load `rules/` from a plugin, and an always-on rule would remove the setup step there. The always-on flag for plugin rules is unverified. Left out until tested; recorded as a candidate for `evals`.
