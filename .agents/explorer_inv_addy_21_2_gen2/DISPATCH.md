...
You are Explorer 2 for work unit `inv-addy-21` (Phase 1 Inventory Extraction).
Your role: Source File Deep-Dive Explorer for `sources/addy-external/idea-refine.md`.
...
Context and Mission:
Investigate `sources/addy-external/idea-refine.md` (24,943 bytes) in full detail for work unit `inv-addy-21` following METHOD.md (rules R1-R6, §4 checklist, §1.1 external docs rules).
...
3. Read `sources/addy-external/idea-refine.md` completely from start to end.
4. Extract all required fields per `docs/plan/templates/inventory-entry.md`:
   ...
   - Phase (required, lifecycle stage in package convention: `addy:Define` or similar, check `docs/analysis/inventory/addy/skills-idea-refine-SKILL-md.md`).
   ...
   - Defects (run METHOD.md §4 checklist: missing-path, script-bug, doc-drift against `skills/idea-refine/SKILL.md`, internal-contradiction, cross-file-contradiction, orphan, etc.).
   - Observations & Context cost.
5. Cross-reference and compare against the repo skill `sources/addy/skills/idea-refine/` (`SKILL.md`, `examples.md`, `frameworks.md`, `refinement-criteria.md`, `scripts/idea-refine.sh`) and its inventory entry `docs/analysis/inventory/addy/skills-idea-refine-SKILL-md.md`. Identify any `doc-drift` where the external page describes the skill differently from its `SKILL.md` (cite both locations per METHOD.md line 35).
6. Compile your findings into `analysis.md` and `handoff.md` in `/Users/peterkloss/Dev/ACMElabs/brain-v2/.agents/explorer_inv_addy_21_2_gen2/`.
