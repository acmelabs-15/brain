# Handoff Report: `inv-addy-6` Focus Extraction (Explorer 1)

## 1. Observation
- Inspected and read `sources/addy/docs/comparison.md` (15,362 bytes, 130 lines) completely from line 1 to 130.
  - Frontmatter/header comment: `"This document is for developers evaluating the project. It is NOT a skill and is not meant to be loaded into an agent's context. It lives in docs/ so it stays out of the agent's working set."` (lines 1-5).
  - Verbatim purpose: `"People often ask how **agent-skills** relates to the two other "skills for coding agents" collections they hear about most: **Superpowers** (Jesse Vincent / obra) and **Matt Pocock's skills**. All three are good, share a lot of DNA, and are worth learning from. This page is an honest map of how they are *shaped* differently, so you can pick the one that fits how you work, or borrow across all three."` (line 9).
  - Architectural comparison: Compares `agent-skills`, `Superpowers`, and `Matt Pocock's skills` across 9 core dimensions (Core idea, Organizing principle, Catalog size, Lifecycle coverage, Entry points, Distinctive mechanisms, Quality measurement, Tooling reach, Governance, Best for) in an "At a glance" matrix (lines 17-29).
  - Persona fan-out assertion: Line 54 states that `/ship` fans out review personas `code-reviewer`, `security-auditor`, `test-engineer`, `web-performance-auditor` in parallel. In contrast, line 98 states: `"/spec to /ship, with the security-auditor and test-engineer personas fanning out at the end."`
  - Cross-file check: In `sources/addy/agents/web-performance-auditor.md:183`, `docs/agents.md:44`, `references/orchestration-patterns.md:54`, and `.claude/commands/ship.md:15`, `/ship` explicitly fans out to only three personas (`code-reviewer`, `security-auditor`, `test-engineer`), deliberately excluding `web-performance-auditor` because performance audits are specific to web applications.
  - Command listing: Line 23 lists `/spec`, `/plan`, `/build`, `/test`, `/review`, `/code-simplify`, `/ship`, plus `/webperf`, omitting `/constraints` (`sources/addy/.claude/commands/constraints.md`).
  - Reference checklist count: Line 54 states that agent-skills "ships seven reference checklists including a Definition of Done", but inspecting `sources/addy/references/` via `ls` shows 5 checklists (`accessibility-checklist.md`, `definition-of-done.md`, `observability-checklist.md`, `performance-checklist.md`, `security-checklist.md`) and 2 pattern documents (`orchestration-patterns.md`, `testing-patterns.md`), totaling 7 reference files.
  - Head-to-head empirical benchmark: Analyzes Om Mishra's comparative study ("Superpowers vs Agent-Skills: Faster Shipping, Safer Reasoning") finding that agent-skills provided faster time-to-code (~8m vs ~12m) and broader validation depth (7 passes catching cross-module regressions vs 5) while Superpowers provided heavier upfront architectural reasoning (lines 60-74).
  - Router stacking rule: Line 119 explicitly warns against running two meta-skill routers simultaneously: `"What does not work is running two of them as your **active router at the same time**. Stacked meta-skills fight over command names (/tdd defined in two places), compete on routing logic, and pull in different TDD philosophies..."`
  - Unsolved frontier: Notes that durable cross-session memory remains unsolved across all three projects (lines 109-113).
- Inspected and read `sources/addy/docs/copilot-setup.md` (3,496 bytes, 88 lines) completely from line 1 to 88.
  - Verbatim purpose: `"Copilot supports creating agent skills using a .github/skills, .claude/skills, or .agents/skills directory in your repository."` (line 7).
  - File extension requirement: Highlights a critical harness-specific constraint: `"> **Important:** GitHub Copilot requires custom agent files to be named *.agent.md. Files named *.md are silently ignored by Copilot."` (lines 23-25).
  - Setup instructions: Recommends copying `skills/test-driven-development/SKILL.md` and `skills/code-review-and-quality/SKILL.md` to `.github/skills/`, and copying `agents/code-reviewer.md`, `agents/test-engineer.md`, and `agents/security-auditor.md` to `.github/agents/*.agent.md` (lines 9-33).
  - Agent omission: Omits `agents/web-performance-auditor.md` (`web-performance-auditor.agent.md`) from the setup commands (lines 30-32) and usage tips (line 85: `"The code-reviewer, test-engineer, and security-auditor agents are designed for Copilot's agent model."`).
  - Chat persona invocation: Demonstrates `@<agent>` chat syntax: `@code-reviewer Review this PR`, `@test-engineer Analyze test coverage for this module`, `@security-auditor Check this endpoint for vulnerabilities` (lines 35-39).
  - Configuration template: Provides a distilled project-level `.github/copilot-instructions.md` configuration template covering Testing (TDD, Prove-It pattern, test hierarchy), Code Quality (5 axes: correctness, readability, architecture, security, performance), Implementation (`implement → test → verify → commit`), and Boundaries (lines 49-76).
  - Conciseness rule: Recommends: `"Keep instructions concise — Copilot instructions work best when focused. Summarize the key rules rather than including full skill files."` (line 84).
- Executed repository validation scripts:
  - `bun scripts/validate-skills.js`: 25 skills checked — 0 error(s), 0 warning(s) — PASSED
  - `bun scripts/validate-commands.js`: 9 commands checked — 0 error(s) — PASSED
...
## 2. Logic Chain
1. From full reading and filesystem verification of `docs/comparison.md`, the document is an architectural positioning guide that categorizes the three major skill libraries:
   - `agent-skills`: Full senior-engineering SDLC lifecycle (Define, Plan, Build, Verify, Review, Ship), review personas, anti-rationalization guards, in-repo 3-tier eval framework, broad platform reach.
   - `Superpowers`: Autonomous, reasoning-heavy inner-loop pipeline, Socratic brainstorming, subagent-driven development with mandatory task reviewer and fix loop, worktree isolation, TDD on skills (`writing-skills`).
   - `Matt Pocock's skills`: Sharp, opinionated Claude Code toolkit, Socratic interrogation loop (`grilling`), seam-based TDD, user vs. model invocation budget separation, tracker integration (`wayfinder`).
2. The comparison document identifies crucial synthesis insights directly relevant to our lifecycle design:
   - The trade-off between **validation depth** (multi-pass verification catching regressions) and **upfront architectural reasoning**.
   - The operational hazard of stacking meta-skill routers simultaneously due to colliding slash commands and conflicting philosophies.
   - The shared industry bottleneck of durable cross-session memory.
3. Cross-file verification exposes two documentation drift defects in `docs/comparison.md`:
   - Line 54 incorrectly claims `/ship` fans out four personas including `web-performance-auditor`, which contradicts `agents/web-performance-auditor.md:183`, `docs/agents.md:44`, `references/orchestration-patterns.md:54`, `.claude/commands/ship.md:15`, and even line 98 of `docs/comparison.md` itself.
   - Line 54 claims seven reference checklists, whereas the repo contains five checklists and two pattern guides.
4. From full reading of `docs/copilot-setup.md`, GitHub Copilot requires specific file system adaptation:
   - Custom agents must be named `*.agent.md` in `.github/agents/` (silently ignored otherwise).
   - Skills can be discovered from `.github/skills`, `.claude/skills`, or `.agents/skills`.
   - Distilling core rules into `.github/copilot-instructions.md` avoids context window saturation.
   - Personas are dispatched via native `@<persona>` chat tagging.
5. In `docs/copilot-setup.md`, `web-performance-auditor` is completely omitted from both the installation commands and the agent model tips, aligning with its exclusion from the standard pre-launch fan-out.

## 3. Caveats
- Both assigned files are documentation documents (`type: doc`); neither is executable as a script nor ships executable code.
- External URLs cited in both files (GitHub documentation, LinkedIn article, external repos) were verified syntactically and semantically against project context, but live web browsing was not performed as read-only local investigation is sufficient.
- The `Scripts` section of the inventory template is populated with `none` as neither document contains executable scripts.

## 4. Conclusion
- Both assigned files (`docs/comparison.md` and `docs/copilot-setup.md`) have been exhaustively read in full from first line to last line and verified against all filesystem paths and cross-references.
- Exact inventory entries complying strictly with `docs/plan/templates/inventory-entry.md` and rules R1-R6 have been generated and recorded below and in `.agents/explorer_inv_addy_6_1/report.md`.
- Concrete documentation drift defects were identified and cataloged with exact `path:line` citations.

## 5. Verification Method
- Independent inspection of source files:
  - `view_file` on `/Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/docs/comparison.md`
  - `view_file` on `/Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/docs/copilot-setup.md`
- Independent verification of referenced paths:
  - `ls -la /Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/agents/`
  - `ls -la /Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/references/`
  - `ls -la /Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/.claude/commands/`
  - `ls -la /Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/skills/`
- Independent execution of validation scripts:
...
- Independent inspection of generated extraction report:
  - `view_file` on `/Users/peterkloss/Dev/ACMElabs/brain-v2/.agents/explorer_inv_addy_6_1/report.md`

---

# Drafted Inventory Entries

## Entry 1: `docs/comparison.md`
