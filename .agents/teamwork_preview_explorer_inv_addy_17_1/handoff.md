# Handoff Report: Explorer 1 (inv-addy-17)

## 1. Observation

### Target File
- **Path**: `/Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/skills/security-and-hardening/SKILL.md`
- **Size**: 24,192 bytes
...
### Direct Text & Code Observations
1. **Purpose Statement**:
   - `skills/security-and-hardening/SKILL.md:10`:
     > "Security-first development practices for web applications. Treat every external input as hostile, every secret as sacred, and every authorization check as mandatory. Security isn't a phase — it's a constraint on every line of code that touches user data, authentication, or external systems."
   - `skills/security-and-hardening/SKILL.md:3`:
...
2. **Phase Assignment**:
   ...
   - `sources/addy/README.md:369`:
     `│   ├── security-and-hardening/        #   Review`
   - `sources/addy/skills/using-agent-skills/SKILL.md:185`:
     `| Review | security-and-hardening | OWASP prevention, input validation, least privilege |`
   - `sources/addy-external/security-and-hardening.md:5`:
     `<span class="crumb-phase" data-astro-cid-jrlgpo3w>Review</span>`, `<span class="chip phase-chip" data-astro-cid-jrlgpo3w><span class="phase-dot" data-astro-cid-jrlgpo3w></span>Review phase</span>`
   - `sources/addy/commands/review.toml:11`:
     `4. **Security** — Input validated? Secrets safe? Auth checked? (Use security-and-hardening skill)`
   - Note: In `sources/addy/AGENTS.md:44`, under `### Lifecycle Mapping (Implicit Commands)`, the Review phase is mapped solely as `- REVIEW → code-review-and-quality`, omitting `security-and-hardening`.

3. **Invoked References and Skills**:
   - `../../references/security-checklist.md` referenced at lines 77, 303, and 451.
     - Existence verified at `/Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/references/security-checklist.md` (11,881 bytes, 206 lines).
   - `observability-and-instrumentation` skill referenced at line 369 ("the `observability-and-instrumentation` skill makes the same point from the ops side").
   - `debugging-and-error-recovery` skill referenced at line 375 ("follow the `debugging-and-error-recovery` skill").
   - External URL referenced: `https://genai.owasp.org/llm-top-10/` at line 379.

4. **Invocations across `sources/addy/`**:
   - `sources/addy/CLAUDE.md:25`
   - `sources/addy/commands/review.toml:11`
   - `sources/addy/README.md:270, 369`
   - `sources/addy/skills/code-review-and-quality/SKILL.md:66, 300`
   - `sources/addy/skills/constraint-driven-development/SKILL.md:10, 310`
   - `sources/addy/skills/observability-and-instrumentation/SKILL.md:91`
   - `sources/addy/skills/source-driven-development/SKILL.md:101`
   - `sources/addy/skills/using-agent-skills/SKILL.md:35, 185`
   - `sources/addy/references/definition-of-done.md:48`
   - `sources/addy/references/observability-checklist.md:30`
   - `sources/addy/references/security-checklist.md:3`
   ...
   - `sources/addy/evals/cases/security-and-hardening.json:2, 35`
   - `sources/addy/evals/cases/context-engineering.json:25`
   - `sources/addy/evals/cases/documentation-and-adrs.json:25`
   - `sources/addy/evals/cases/idea-refine.json:25`
   - `sources/addy/evals/fixtures/security-and-hardening/webhook.js` & `webhook.test.js`
   - `sources/addy-external/security-and-hardening.md:1, 5, 8`

5. **Scripts**:
   - No `scripts/` directory under `sources/addy/skills/security-and-hardening/`.
   - Contains a bash one-liner at line 351: `git diff --cached | grep -i "password\|secret\|api_key\|token"`.
   - Evaluated exit behavior: exits 1 when no matching secrets found (normal grep behavior).
...
1. **Phase Classification**:
   - Observation 2 demonstrates that `CLAUDE.md`, `README.md`, `skills/using-agent-skills/SKILL.md`, and `sources/addy-external/security-and-hardening.md` uniformly classify `security-and-hardening` under `Review`.
   - Therefore, the package lifecycle phase is canonically `addy:Review`.
   - Although line 10 asserts "Security isn't a phase — it's a constraint on every line of code...", the package's structural categorization places its primary evaluation gate at the Review stage.

2. **Boundary & Defense Architecture**:
   - The skill establishes a multi-layered defensive posture:
     1. Threat modeling via STRIDE (lines 21-40) and abuse-case generation prior to code writing.
     2. Three-tier decision framework (Always Do, Ask First, Never Do) at lines 42-74.
     3. OWASP Top 10 web defenses (injection, auth, XSS, access control, SSRF) with TypeScript code patterns.
     4. Triaging dependency audit advisories by reachability and execution context rather than blind automation (lines 272-311).
     5. Data privacy engineering (classification, retention TTL, export/deletion paths for GDPR/CCPA) at lines 356-376.
     6. Modern AI/LLM hardening based on OWASP Top 10 for LLM Applications (2025) at lines 377-404.

3. **Defects Identification**:
   - `doc-drift`: `sources/addy/AGENTS.md:44` maps the Review lifecycle stage solely to `code-review-and-quality`, omitting `security-and-hardening`, `code-simplification`, and `performance-optimization`, contradicting `CLAUDE.md:25` and `README.md:369`.
   - `doc-drift`: `sources/addy-external/security-and-hardening.md:10` claims a standardized 6-section anatomy (`Overview`, `When to Use`, `Process`, `Rationalizations`, `Red Flags`, `Verification`), whereas the actual file contains 13 top-level sections including extensive domain-specific technical guides.
   - `doc-drift`: `commands/review.toml:11` embeds `security-and-hardening` as axis 4 of `/review`, but no dedicated standalone slash command (e.g. `/security`) exists in `sources/addy/commands/`, whereas `sources/addy-external/security-and-hardening.md:5` associates it directly with the `/review` command.

4. **Reference Completeness**:
   - All relative file paths referenced in `security-and-hardening/SKILL.md` exist on disk. No missing paths.

---

## 3. Caveats

- **External doc generation**: `sources/addy-external/security-and-hardening.md` was fetched during Phase 0 setup from `https://skills.addy.ie/skills/security-and-hardening/`. The HTML content represents Addy's web presentation, which uses template cards that summarize the skills rather than rendering the raw markdown.
- **SSRF TOCTOU Gap**: As explicitly documented in the file at line 220, the provided DNS resolution pattern in lines 201-216 mitigates simple SSRF but retains a TOCTOU DNS rebinding risk if standard `fetch` is used without an IP-pinned socket connection or reverse proxy filter.
- No other caveats.
...
The skill `sources/addy/skills/security-and-hardening/SKILL.md` is a fully developed, high-quality skill definition covering modern web security, supply-chain integrity, privacy compliance, and LLM-specific vulnerabilities.

Here is the complete, ready-to-persist draft inventory entry for `docs/analysis/inventory/addy/skills-security-and-hardening-skill-md.md`:
...
- external-doc sources/addy-external/security-and-hardening.md — sources/addy-external/security-and-hardening.md:1, sources/addy-external/security-and-hardening.md:5, sources/addy-external/security-and-hardening.md:8
...
- Security Review Checklist (line 405)
  - [Checklist sections within template codeblock: Authentication (line 408), Authorization (line 414), Input (line 419), Data (line 425), Infrastructure (line 433), Supply Chain (line 439), AI / LLM (if used) (line 444)]
- See Also (line 449)
...
1. **Full File Read & Size Verification**:
   - Inspect `/Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/skills/security-and-hardening/SKILL.md` using `view_file` to verify line count (500 lines) and exact text matches.
   - Run `wc -c /Users/peterkloss/Dev/ACMElabs/brain-v2/sources/addy/skills/security-and-hardening/SKILL.md` to confirm byte count is exactly 24,192 bytes.

2. **Phase Cross-Referencing**:
   - Inspect `sources/addy/CLAUDE.md:25` and `sources/addy/README.md:369` to verify `addy:Review` phase categorization.
   - Inspect `sources/addy/AGENTS.md:44` to verify the documented `doc-drift` discrepancy.

3. **Invocations and Existence Checks**:
   - Run `grep -rn "security-and-hardening" sources/addy/` to confirm all citing files.
   - Run `ls -la sources/addy/references/security-checklist.md` to verify the referenced checklist exists and check its size (11,881 bytes).

4. **Invalidation Conditions**:
   - The findings are invalidated if `security-and-hardening/SKILL.md` is modified, if a new command `/security.toml` is added, or if `references/security-checklist.md` is deleted or moved.
