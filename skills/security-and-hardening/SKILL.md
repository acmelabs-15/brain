...
These are prevention patterns, not a ranking. For the 2021 ordering, see the quick-reference table in `~/.claude/references/security-checklist.md`.
...
When data crosses a trust boundary, validate it as untrusted (see Input Validation above); when a privacy incident exposes personal data, the breach-notification clock is part of the postmortem — follow the `diagnosing-bugs` skill.
...
For detailed security checklists and pre-commit verification steps, see `~/.claude/references/security-checklist.md`.
