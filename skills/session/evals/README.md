...
`tier-sweep/` measured the same evals on Haiku, Sonnet and Opus; `trigger/` measured the
description against `trigger-eval.json` — note that `trigger/tiers/*.results.json` record the
**pre-plugin** description (it names envsetup and the run skills), so the current description is
unmeasured on Sonnet and Haiku, where the recorded sweep found 6/10 and 2/10 should-fire recall.
