---
name: session
argument-hint: "start [PLAN-NNN] | entry | end | close"
...
description: "Runs the session ritual of a repo's docs system and produces its artifacts: `/session start [PLAN-NNN]` reads the docs system in full (OVERVIEW, the plan and its PRD, every open docs/sessions/SES-NNN serving it, CONTEXT.md, the ADRs it cites), joins the open session serving the plan part or opens one, marks the part in progress, and posts a brief; `/session entry` after every commit appends and fills the commit's entry, updates what the change made stale (plan ticks, OVERVIEW, ADR, PRD, CONTEXT.md) and commits it as docs(session); `/session end` leaves the session open with the log complete and a handoff written; `/session close` closes a session whose Goal is done and marks its plan part done. Use at the start of a conversation in a repo with docs/sessions (or to set one up: `session init`), right after each commit, and before finishing. Not for authoring the docs themselves (an ADR, a PRD, a plan, the glossary — their own skills) and not for changing the tool behind it (the plugin README)."
---
...
commit; a conversation that changes nothing needs none. `references/docs-system.md` is the record
itself — what a plan part's status line, a session file, an entry and an `Also:` line look like —
for the moment a step below names a shape you have not seen.

Arguments: **$ARGUMENTS** — a mode (`start`, `entry`, `end`, `close`) and, for `start`, an optional
`PLAN-NNN`. When the mode is empty, infer: no session joined or opened in this conversation →
`start`; a commit just landed → `entry`; the user is wrapping up and the Goal is not done → `end`;
the Goal is done → `close`. The typed-only aliases `/session-start`, `/session-entry`,
`/session-end`, `/session-close` invoke the same modes.

The tool is one exact command, always written this way (quoted path, then the subcommand):

```bash
bun "${CLAUDE_PLUGIN_ROOT}/skills/session/scripts/session.ts" <init | list | new | append | current | check | close> …
```

It is written `session <subcommand>` below. It finds the repo on its own (`CLAUDE_PROJECT_DIR`,
else the git toplevel), so it runs from any directory; paths below are relative to the repo root.
What each subcommand prints, and every refusal with its cause, is `references/tool.md` — read it
when the tool says something a step below does not predict.
...
- **A Sessions line that says `no session log at …`** means the repo has no docs system yet: run
  ...
  section of `CONTEXT.md`, keeping any file that exists), commit that, and continue with `start`.
...
- **Your session is the one you joined or opened, never "the newest".** Another conversation may
  ...
  warnings, and you leave that file untouched.
...
- **`new` opens a session for a stream of work, not for a conversation.** A conversation
  continuing that work joins with `--session`. One session per plan part; a second conversation
  on the same part joins the part's session.
...
- [ ] 2 the plan read to the end (+ the PRD it cites), or the item chosen from OVERVIEW Next up
...
2. The plan. With `PLAN-NNN` given: `docs/plan/PLAN-NNN-*.md` in full — every part's status line
   (`planned | in progress (session SES-NNN) | done (session SES-NNN, sha)`) tells you where the
   plan stands and which session holds the story of each part — then the PRD its Overview cites,
   in full. Without one: OVERVIEW's **Next up** names the item; if it points at a plan, read that
   plan the same way.
...
6. Every `ADR-NNN` the plan (or the item) cites, in the repo's decisions directory (settled — a
   ...
   re-research; `grep -rn <file-or-keyword> docs/sessions/`. A directory's own `CLAUDE.md` loads
   when you read files there.
7. The session, one of three outcomes:
   - **join** — the part you take is `in progress (session SES-NNN)` and that session is open: it
     is yours from here on (`--session SES-NNN`), say so in the brief. A session another
     conversation owns — its placeholders unfilled, its Outcome not yours to write — is never
     joined, whatever its title says; and a question that changes nothing is `none`, not a join;
   - **open** — the part is `planned` (or the work has no plan):
     `session new <slug> --plan "PLAN-NNN · part N"` (slug = the work ahead, kebab-case; omit
     `--plan` only for unplanned work), then set the title and `Goal` in the file it names, and
     change the part's status line to `in progress (session SES-NNN)` — that line is how the next
     conversation finds this session;
...
1. `session append --session SES-NNN` appends one skeleton per commit not yet accounted for
   ...
   commits. The table of which commits get no entry is in `references/docs-system.md`.
2. Fill every placeholder. `Summary` = what the change does as a whole; `Why` = the problem or
   ...
   or "changes" is not a phrase, and `references/docs-system.md` shows a filled line beside its
...
2. In your session file: `Open at end` says what the next conversation picks up first and what is
   ...
   `in progress (session SES-NNN)` — that is the pointer the next conversation follows.
...
4. `session close --session SES-NNN` — it runs the gate first, this time counting `Outcome` and
   ...
   a problem. Then stage by name and commit `docs(session): close SES-NNN`, and run `end` step 5
   (the tree).
