...
| --- | --- | --- |
| PLAN-001-<feature>.md | requirements … | planned \| in progress — part N (session SES-NNN) \| done — shipped in vX.Y.Z |
```

The **Plans** table is the spec's way down: every plan that implements it, with the plan's status,
so spec → plan → part → session is one walk for a conversation that starts from nothing. Keep it
current in the same change that moves a plan's status (the `/session` skill does this where a
repo has a session log); a spec whose table lags its plans sends the next conversation to the
wrong place.
...
5. Define verification checkpoints between parts, and give every part its `> Status: planned` line
   (`planning-and-task-breakdown` carries the shape; a session log turns those lines into pointers)
