...
describe("applyUpstream", () => {
  test("writes every planned file, the licence, and a lock entry per file", async () => {
    const v1 = await tree("v1", { "skills/a/SKILL.md": "a1\n", "skills/b/SKILL.md": "b1\n", LICENSE: "MIT\n" });
    const root = `${work}/brain1`;
    await mkdir(root, { recursive: true });
    const lock = emptyLock();
    const units = await planUnits("addy", [{ from: "skills", to: "skills" }], v1);
    const result = await applyUpstream({ name: "addy", units, tree: v1, license: "LICENSE", lock, root });
    ...
    expect(result.written.sort()).toEqual(["licenses/addy.LICENSE", "skills/a/SKILL.md", "skills/b/SKILL.md"]);
  ...
  test("a second apply updates changed files, adds new ones, deletes dropped ones, and leaves brain-owned files alone", async () => {
    ...
    const lock = emptyLock();
    await applyUpstream({ name: "addy", units: await planUnits("addy", [{ from: "skills", to: "skills" }], v1), tree: v1, lock, root });
    const result = await applyUpstream({ name: "addy", units: await planUnits("addy", [{ from: "skills", to: "skills" }], v2), tree: v2, lock, root });
    ...
    expect(Object.keys(lock.files).sort()).toEqual(["skills/a/SKILL.md", "skills/c/SKILL.md"]);
  });
  ...
  test("touches nothing of another upstream", async () => {
    ...
    await mkdir(root, { recursive: true });
    const lock = emptyLock();
    await applyUpstream({ name: "addy", units: await planUnits("addy", [{ from: "skills", to: "skills" }], addy), tree: addy, lock, root });
    await applyUpstream({ name: "matt", units: await planUnits("matt", [{ from: "eng/dm", to: "skills/domain-modeling" }], matt), tree: matt, lock, root });
    const empty = await tree("addy3-empty", { "skills/.keep": "" });
    await rm(`${empty}/skills/.keep`);
    await applyUpstream({ name: "addy", units: await planUnits("addy", [{ from: "skills", to: "skills" }], empty), tree: empty, lock, root });
