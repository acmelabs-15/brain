...
async function synced(name: string) {
  const up = await tree(`${name}-up`, { "skills/a/SKILL.md": "a\n", "skills/a/ref.md": "r\n", "references/x.md": "x\n" });
  const root = `${work}/${name}-brain`;
  await mkdir(`${root}/skills/own`, { recursive: true });
  await Bun.write(`${root}/skills/own/SKILL.md`, "mine\n");
  const lock = emptyLock();
  const units = await planUnits("addy", [{ from: "skills", to: "skills" }, { from: "references", to: "references" }], up);
...
describe("checkTree", () => {
  ...
  test("findings come sorted by path", async () => {
    ...
    await rm(`${root}/references/x.md`);
    expect((await checkTree(root, lock)).map((d) => d.path)).toEqual(["references/x.md", "skills/a/ref.md"]);
  });
});
