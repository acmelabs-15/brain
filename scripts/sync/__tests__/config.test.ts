...
describe("parseConfig", () => {
  test("accepts the committed upstream.json", async () => {
    ...
    expect(Object.keys(config.upstreams)).toEqual(["agent-skills", "mattpocock-skills", "ask-user-question"]);
  ...
  test("rejects a missing sha, naming the upstream", () => {
    ...
    expect(() => parseConfig(raw)).toThrow('upstream "agent-skills": sha must be 40 hex characters');
  });
  ...
  test("rejects a take target outside the allowed roots, naming the path", () => {
    ...
    expect(() => parseConfig(raw)).toThrow('upstream "agent-skills": take target "docs" is outside the allowed roots');
  });

  test("rejects a seed target outside the allowed roots", () => {
    const raw = valid();
    raw.upstreams["agent-skills"].seed = [{ from: "evals", to: "evals" }];
    expect(() => parseConfig(raw)).toThrow('upstream "agent-skills": seed target "evals" is outside the allowed roots');
  });
  ...
  test("rejects a path with .. in it", () => {
    ...
    expect(() => parseConfig(raw)).toThrow('upstream "agent-skills": path "../etc" must be relative and inside the repo');
  });
});
