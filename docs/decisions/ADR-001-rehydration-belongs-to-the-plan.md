# ADR-001: Rehydration belongs to the plan skill; the session skill keeps the record with three acts and a status
...
text also stands as ADR-024. What stands from those: the session as a stream of work (ADR-020), the
entry grain (ADR-021), the plan part → session pointer and the PRD Plans table (ADR-022).
...
the context, including how to identify the correct session." ANA-001 read every installed skill,
...
- PLAN-001 in this repo (begun as PLAN-002 in acmelabs-15/sessions) carries the work in five parts (the record's model; the
