...
The next conversation (2026-09-01) took the handover's threads. The width question went to Peter with the measurement — a chip's room is the card width less 140 px, so Build's 160–182-wide loop cards held three to six characters — and his first reply ("already done and works correctly") did not match the artifacts, which I checked: the live page was byte-identical to the repo's build and the canvas carried the same five rows. A re-ask through the dialog got `/wait-what`; the re-pitch in plain prose landed, and he took the recommendation: widen to 284 and redraw Build on three rows (`88c6a97`). Rejected on the way: terse chips at today's widths, rows only where 220 px fit, and freeing 45 px by taking the selected-dot out of the flex flow. The DevTools bridge's shared Chrome profile was held by the previous conversation's server; its idle browser was stopped so this one could launch.
...
### 2026-09-01 · docs(lifecycle): INPUT / OUTPUT rows through Plan, Build, Review and Release — the stage cards share one width (284, three columns, the narrowest that holds docs/plan/PLAN-NNN.md in a chip), Build redrawn on three rows with its return loops in the corridors, tick · session log on one line; both artifacts republished from the same paths; the README's model notes and open threads and PLAN-005's open question follow · 88c6a97

- Summary: the INPUT / OUTPUT rows (label left, mono chip right, inside the measured header) now run through every stage's cards — Plan's quiz round, the walk, read-its-session and session start; Build's command, implement, test-driven-development, suite + build, doubt-driven-development, diagnosing-bugs, tick · session log, part close and converge; Review's and Release's five. To hold them the stage cards share one width, `CW` = 284 (a chip's text room is the card width less 140 px at 6 px a character; 284 is the widest three-across beside the panel and the narrowest that holds `docs/plan/PLAN-NNN.md`, which ellipsized at the planner's old 260), three columns `COLS` and a `GAP` of 36; Build is redrawn on three rows (the forward path; suite → commit → tick right-to-left; part close and converge, with doubt / diagnosing-bugs under the suite) and its return loops ride the corridors between rows, the strip right of the third column and the bottom-left edge; `tick · session log` sits on one line at 284. Entries (the document, the brief, the commit) keep their subtitles; the concurrent view is untouched. Both artifacts republished from the same paths (canvas contract 0.1.31, 🧠; page 🗺️).
- Why: Peter, 2026-09-01, the handover's three threads — carry the rows through the other stages, fix the tick title's wrap, republish. The width question went to him with the measurement (Build's loop cards held 3–6 characters at 160–182 px); after a /wait-what re-pitch he took the recommendation: widen and redraw Build rather than terse or ellipsized chips.
- Files:
  - `docs/lifecycle/Main.dc.html` (+82/−66) — `CW` / `GAP` / `COLS` statics; `viewPlan`, `viewBuild`, `viewReview` and `viewRelease` rewritten on the three columns with `io` rows in place of subtitles; Build's `cy1/cy2/cy3` rows, the two corridors (`cor1`, `cor2`), the right strip (`RC`) and the bottom line for the loops; the next-task loops now return to `/brain:build` (directly above the tick); Plan's waits note and brief fit the strip beside the panel
  - `docs/lifecycle/README.md` (+9/−3) — the model gains the width rule and Build's three-row layout; the two open threads this commit closes are replaced by the one it leaves (the concurrent view's subtitles wait on Part 7 Task 2)
  - `docs/plan/PLAN-005-lifecycle-composition-implementation.md` (+3/−2) — the interactive map's open question: the rows run through every stage since 2026-09-01; placement still Part 8's
- Notes: verified — Chrome via the DevTools MCP at 1600×1060×1, the page driven by script: 17 view × switch states checked for chip overflow, wrapped titles, card overlaps, labels over cards and anything past the panel or the legend — all clean (two defects found and fixed on the way: waits over the quiz card; session start's OUTPUT chip 7 px over, the OUTPUT label being wider than INPUT); six states screenshotted and read. `node build-page.mjs` and the canvas seed + `--check` clean; both artifacts republished (the page needed the live copy read in full first — it was byte-identical to the previous build). Unverified — nothing; no plugin file changed, so the plugin checks were not run. Follow-ups: rows on the concurrent view when Task 2 lands; the map's home (Part 8).
