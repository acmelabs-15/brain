...
- Practical authoring patterns from across the 25 catalog skills (docs/skill-anatomy.md:37-73, 115-117)
- Bash automation and safety scripting standards (shebang, `set -e`, stderr logging, stdout JSON, temp trap) (docs/skill-anatomy.md:131-140)
- LLM context efficiency and token budget constraints (<500 lines for `SKILL.md`, <1024 chars for description, 1-level deep reference linking) (docs/skill-anatomy.md:33, 121-129)

## Outputs — required
- Canonical skill file layout schema: `skills/<skill-name>/SKILL.md` (required), `scripts/` (optional), `references/` (optional), `supporting-file.md` (optional) (docs/skill-anatomy.md:7-18)
- YAML frontmatter schema contract: `name` (lowercase kebab-case, must match directory), `description` (third-person summary + positive "Use when" trigger clause, <=1024 chars) (docs/skill-anatomy.md:22-35)
...
- Section purpose definitions and behavioral guidelines (docs/skill-anatomy.md:75-98)
- Supporting files threshold criteria (>100 lines for supporting docs, <50 lines inline) (docs/skill-anatomy.md:100-109)
- Shared references architectural convention (centralized root `references/` for multi-skill checklists vs. colocated skill references) and portability trade-off (docs/skill-anatomy.md:111-119)
- Five context efficiency optimization rules (docs/skill-anatomy.md:121-129)
- Six runnable script authoring requirements (docs/skill-anatomy.md:131-140)
- Six core writing principles: Process over knowledge, Specific over general, Evidence over assumption, Anti-rationalization, Progressive disclosure, Token-conscious (docs/skill-anatomy.md:142-149)
- Naming conventions across skill directories, skill files, supporting files, shared references, and skill-specific references (docs/skill-anatomy.md:151-157)
- Cross-skill reference syntax pattern (`[a-z0-9-]+` skill mention) (docs/skill-anatomy.md:159-168)
- Required vs. recommended specification boundary (docs/skill-anatomy.md:170-182)
...
- `missing-path` · `docs/skill-anatomy.md:117` · Acknowledges an unresolved packaging dependency hazard (tracked in GitHub issue #361) where per-skill installation tooling copies only `skills/<name>/`, leaving shared root `references/` behind and causing broken markdown links at runtime.
