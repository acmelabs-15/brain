...
## Phase — required
`addy:Build` (rendered as `Build` in breadcrumb at `../addy-external/test-driven-development.md:5` and as `Build phase` in the phase chip and sidebar at `../addy-external/test-driven-development.md:5, 12`). Note: Matches `CLAUDE.md:23` and `README.md:363` (`Build`), but directly conflicts with the authoritative meta-skill `skills/using-agent-skills/SKILL.md:180`, which classifies `test-driven-development` under `Verify`.
...
- Static assets, stylesheets, Netlify RUM analytics script, and site navigation links (`/skills/`, `/docs/getting-started/`, `/tutorials/`, `/lifecycle/`, `/loops/`, `/teach/`, `/compare/`) — ../addy-external/test-driven-development.md:1, 5, 12, 15, 19
...
- doc lifecycle — ../addy-external/test-driven-development.md:5, 12, 15
- doc docs/getting-started — ../addy-external/test-driven-development.md:5, 15
- doc tutorials — ../addy-external/test-driven-development.md:5, 15
- doc loops — ../addy-external/test-driven-development.md:5, 15
- doc teach — ../addy-external/test-driven-development.md:5, 15
- doc compare — ../addy-external/test-driven-development.md:5, 15

## Invoked by — required
orphan — This static external HTML snapshot (`sources/addy-external/test-driven-development.md`) is a frozen snapshot of `https://skills.addy.ie/skills/test-driven-development/` brought into scope via METHOD.md §1.1. No in-scope repository file in `sources/addy` links to or loads this static external HTML snapshot file directly.
...
- `npx skills add addyosmani/agent-skills --skill test-driven-development` — ../addy-external/test-driven-development.md:8 — defined here
- `npx skills add addyosmani/agent-skills` — ../addy-external/test-driven-development.md:8, 15 — defined here
- `How this skill is structured` — ../addy-external/test-driven-development.md:8 — defined here
...
- `Read the full SKILL.md` — ../addy-external/test-driven-development.md:11 — used here
...
- `See the full lifecycle` — ../addy-external/test-driven-development.md:12 — used here
- `More in the Build phase` — ../addy-external/test-driven-development.md:12 — defined here
...
- `Contributing` — ../addy-external/test-driven-development.md:15 — used here
- `Evals framework` — ../addy-external/test-driven-development.md:15 — used here

## Structure
- `test-driven-development` (H1 — line 5)
...
- `More in the Build phase` (H2 — line 12)
  ...
  - `source-driven-development` (H3 — line 12)
- `Product` (H3 — line 15)
- `Setup` (H3 — line 15)
...
## Defects — required
- `internal-contradiction` / `doc-drift` · `../addy-external/test-driven-development.md:5` vs `../addy-external/test-driven-development.md:12` · **Self-Contradictory Command Binding**: In the page header badge (`line 5`), the command is displayed as `<span class="detail-cmd-badge mono">/test</span>`. However, in the page sidebar (`line 12`), the command is listed as `<div class="side-row"><span>Command</span> <code class="mono">/build</code></div>`. The page directly contradicts itself on which command executes the skill.
- `cross-file-contradiction` / `doc-drift` · `../addy-external/test-driven-development.md:12` vs `sources/addy/commands/test.toml:4` and `sources/addy/commands/build.toml:4` · The sidebar's assignment of `/build` to `test-driven-development` contradicts in-repo command definitions. `/test` executes `skills/test-driven-development/SKILL.md`, whereas `/build` executes `skills/incremental-implementation/SKILL.md`. This was caused by the web page inheriting the generic Build phase template without overriding the command field.
- `cross-file-contradiction` / `doc-drift` · `../addy-external/test-driven-development.md:5` vs `sources/addy/skills/using-agent-skills/SKILL.md:180` · The external page classifies `test-driven-development` under `Build phase`, in agreement with `CLAUDE.md:23` and `README.md:363`. However, the repository's authoritative meta-routing skill, `skills/using-agent-skills/SKILL.md:180`, places `test-driven-development` under `Verify`.
- `doc-drift` · `../addy-external/test-driven-development.md:5` vs `sources/addy/skills/test-driven-development/SKILL.md:3, 10` · The external page summary describes the skill as `"Drives development with tests. Red-Green-Refactor, test pyramid (80/15/5), test sizes, DAMP over DRY, the Beyoncé Rule, and browser testing. Tests are proof, not an afterthought."`, whereas in-repo `SKILL.md:3` defines description as `"Drives development with tests. Use when implementing any logic, fixing any bug, or changing any behavior. Use when you need to prove that code works, when a bug report arrives, or when you're about to modify existing functionality."` and `SKILL.md:10` provides an overview without mentioning the specific roster of techniques.
- `doc-drift` · `../addy-external/test-driven-development.md:5` vs `sources/addy/skills/test-driven-development/SKILL.md:12-20` · The external page condenses "Use when" to `"Implementing logic, fixing bugs, or changing behavior."`, dropping the five structured positive criteria (including edge case handling and risk of regression) and omitting the negative boundary constraint (`**When NOT to use:** Pure configuration changes, documentation updates, or static content changes that have no behavioral impact.`).
- `doc-drift` / `cross-file-contradiction` · `../addy-external/test-driven-development.md:10` vs `sources/addy/skills/test-driven-development/SKILL.md:24-387` · The external page asserts a 6-part standardized skill anatomy (`01 Overview`, `02 When to Use`, `03 Process`, `04 Rationalizations`, `05 Red Flags`, `06 Verification`), but `SKILL.md` has no `Process` section, features multiple intermediate technical sections (`Discover the Stack First`, `The TDD Cycle`, `The Prove-It Pattern`, `The Test Pyramid`, `Writing Good Tests`, `Test Anti-Patterns to Avoid`, `Browser Testing with DevTools`, `When to Use Subagents for Testing`, `See Also`), and titles its rationalizations section `## Common Rationalizations` rather than `Rationalizations`.
- `doc-drift` · `../addy-external/test-driven-development.md:8` vs `sources/addy/docs/skill-anatomy.md:111-119` · Promotes isolated installation via `npx skills add addyosmani/agent-skills --skill test-driven-development`, which drops shared reference file `references/testing-patterns.md`, resulting in broken links at runtime.
- `other` · `../addy-external/test-driven-development.md:1, 5` vs `sources/addy/skills/test-driven-development/SKILL.md:161` · Orthographic variation in naming: the external document uses `Beyoncé Rule` (with acute accent é) in metadata and summary text, whereas `SKILL.md:161` spells it `Beyonce Rule` (unaccented e).
