...
Index and category manifest for the `skills/misc/` bucket. Catalogs specialized or unpromoted tools kept in the repository for specific administrative or course-building workflows, providing one-line descriptions and links to their respective `SKILL.md` documents.
...
- `Misc` — skills/misc/README.md:1 — defined here
...
## Observations
Per `CLAUDE.md:9` and `CLAUDE.md:15`, skills in `misc/` are non-promoted bucket tools: they do not appear in top-level `README.md` or `.claude-plugin/plugin.json`, get no docs pages, and use a flat list in their bucket README rather than the User-invoked / Model-invoked split.

## Context cost
684 bytes, 9 lines, approximately 150 tokens.
