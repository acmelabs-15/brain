...
Declarative OpenAI agent interface descriptor for the `triage` skill. Provides human-readable display metadata ("Triage") and functional summary for OpenAI agent environments. Configures `policy.allow_implicit_invocation: false` to ensure the skill is never triggered implicitly or autonomously by the model, matching `disable-model-invocation: true` in `triage/SKILL.md`.
...
- `allow_implicit_invocation` — skills/engineering/triage/agents/openai.yaml:5 — used here
