...
Declarative OpenAI agent interface configuration for `writing-fragments` in `skills/in-progress/`. Sets display name ("Writing Fragments") and short description ("Mine raw writing fragments"), setting `allow_implicit_invocation: false` to require explicit invocation rather than autonomous triggering.
...
- `Writing Fragments` — skills/in-progress/writing-fragments/agents/openai.yaml:2 — defined here
- `short_description` — skills/in-progress/writing-fragments/agents/openai.yaml:3 — used here
- `allow_implicit_invocation` — skills/in-progress/writing-fragments/agents/openai.yaml:5 — used here
