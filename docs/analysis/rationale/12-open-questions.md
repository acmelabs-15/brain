...
|---|---|---|---|
| 1 | Where is Antigravity's concurrent-stream limit? | The step-up rule: a probe of more units or more simultaneous runs whose wall time grows by more than 25% at the same unit caps. 11 simultaneous conversations produced no error in run 02. | The limit becomes `max_clean_run` / `max_clean_concurrency` in `budget-params.json`; the series stops there. |
| 2 | Does the 5-hour quota bucket bind before the stream limit? | 429s in the run's files (`progress.md`, worker handoffs) and `await-run.ts` STALLED. | The primary agent waits through the pause; Teamwork's Sentinel resumes the workers when the bucket refills (Peter, 2026-09-05, correcting an earlier roll-back rule; sessions 006–007 observed a four-hour pause that resumed). A paused run counts for quality but not as a step-up measurement. |
