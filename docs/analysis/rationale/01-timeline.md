...
- **Sessions 011–012**: the Phase 1V re-run (`units.ts rerun`, runs 18–19) and re-verification; Phase 1 closed; `STATE.md` to Phase 2.
- **Session 013** stopped for Peter in two minutes with the right diagnosis: Phase 2 had no unit system — the inventory names 13,765 things and every tool was bound to `manifest/units.md`. → **D-023** (the Phase 2 kit): `partition-concepts.ts` (460 concept units of ≤ 30 slugs), the unit tools reading both manifests, `unit-facts` and `memo` for concept units, `memo.ts stamp-unit` / `verify` (one call per unit instead of thirty edits), `concept-index.ts`, `kind: name-only`, the step-up series restarted for the new kind of unit.
