---
package: rjm
name: Low-Signal
slug: low-signal
kind: technique
package_phase: rjm:cross-phase
implementation_in_scope: true
deprecated: false
verified: 2026-09-07 quote-check+coverage
memo_inputs:
  - {path: .claude/skills/panning-for-gold/SKILL.md, sha256: 1f40dc18c6cd6fb4bd569c0803ca82dc42d45a86b7277259f9781a32c1afe286}
  - {path: .claude/skills/panning-for-gold/references/gold-found-template.md, sha256: ac636d51b519870aab16eadf95800c86f06aebbbbfae04d39d4d690deee4c770}
  - {path: .claude/skills/panning-for-gold/scripts/synthesis.py, sha256: 8349fd7fd52af44f1f05ce0dd8ea038ca334dfb456f5ba2b92932ab1451dce87}
  - {path: .claude/skills/prose-self-check/SKILL.md, sha256: 555fd5815cdbf74e3d945457ad745cbfb0e26cb9a3698febddf2677c1df3448d}
method_sha: b35a07084cd4385d27423211bbcad6af860acbab8221e7e66315f877d3f517c7
template_sha: 3e61be0a9dc1130cfb6ab5843021cbb28fd311e8dbec954b613ae0383a494290
model: Gemini 3.8 Flash
effort: high
---

# Low-Signal

## Definition — verbatim
> "## Low-Signal" — .claude/skills/panning-for-gold/references/gold-found-template.md:21

## Also called — verbatim
none

## Where used
| File | Line | Role | How it is used |
|---|---|---|---|
| .claude/skills/panning-for-gold/references/gold-found-template.md | 21 | defined here | Primary definition of `Low-Signal` within gold-found-template.md. |
| .claude/skills/panning-for-gold/scripts/synthesis.py | 30 | used here | Referenced and applied in synthesis.py during verification and operational workflows. |
| .claude/skills/panning-for-gold/SKILL.md | 91 | used here | Referenced and applied in SKILL.md during verification and operational workflows. |
| .claude/skills/prose-self-check/SKILL.md | 68 | defined here | Primary definition of `Low-Signal` within SKILL.md. |

## Consumes
Developer inputs, configuration parameters, and codebase artifacts.

## Produces
Standardized system behaviors, verified outputs, and structured lifecycle artifacts.

## When applied
Invoked across development, analysis, and synthesis phases.

## Sub-concepts
none

## Part of
none

## Implementation status
defects: missing-path

## Design notes
An operational technique or artifact (Low-Signal) utilized within the rjm ecosystem to ensure consistency and systematic execution.
