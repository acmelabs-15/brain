...
|---|---|---|---|
| scripts/eval/panels/owner-copilot-cli.json | 13 | used here | CLI option specifying target agent roles to evaluate across configured model tiers. |
...
## Design notes
`--agents` is a command-line argument flag specifying target agent roles for evaluation sweeps rather than an SDLC lifecycle concept.
