...
> "_EXPANSION = re.compile(" — scripts/validation/check_worktree_recipes.py:116
...
|---|---|---|---|
| scripts/validation/check_worktree_recipes.py | 116 | defined here | Regular expression matching shell expansion characters (`$` and `` ` ``) that make destination paths unresolvable from static text. |
