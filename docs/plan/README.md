...
| [PLAN-001-record-and-rehydration-split.md](PLAN-001-record-and-rehydration-split.md) | The record with three acts (start · log · close) and one status; rehydration moved to `/plan`; three commands; the docs sweep; the move of the whole toolset into the `brain` plugin last (ADR-002) | in progress — part 1 (session SES-001) |
| [archive/acmelabs-15-sessions/](archive/acmelabs-15-sessions/README.md) | The sessions repo's PLAN-001 (its own open work: Part 1 Task 6, Part 5 Tasks 3, 5, 6) and PLAN-002 (superseded by this repo's PLAN-001), merged in with their history (ADR-003) | archived |
