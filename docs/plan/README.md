...
| [PLAN-001-record-and-rehydration-split.md](PLAN-001-record-and-rehydration-split.md) | The record with three acts (start · log · close) and one status; rehydration moved to `/plan`; three commands; the docs sweep; the toolset copied into the `brain` plugin at Part 2 and its sources retired at Part 6 (ADR-002, ADR-003, ANA-003) | in progress — Parts 1–3 done (sessions SES-001–003); Part 4 next |
