...
| [PLAN-002-consult-skills-integration.md](PLAN-002-consult-skills-integration.md) | `domain-modeling` and `codebase-design` evaluated in full against the 24 Addy lifecycle skills and 8 commands; the collision matrix, the priced integration options, Peter's decisions into ADR-005, then the implementation | in progress — Parts 1–4 done (SES-006); Part 5 partly waits on PLAN-003 |
| [PLAN-003-define-plan-synthesis.md](PLAN-003-define-plan-synthesis.md) | Define and Plan across both lineages: the Matt collection completed, the overlap mapped at depth with external comparanda (ANA-008), the best parts composed at section grain into a new skill set compatible with brain (ANA-009), Peter's decisions into an ADR, then implementation (Peter, 2026-08-31) | planned |
