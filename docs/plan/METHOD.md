...
7. **The numbers are D-010.** Units per run (`n`), runs at once (`k`) and the ceiling come from `budget.ts`, whose parameters are measured and stored in `docs/plan/budget-params.json`; the step-up rule in §8.2 is how they grow. Concurrency *inside* a run (workers per run) is the Orchestrator's and has always been every unit at once.
...
8. Run and record output and exit codes: `bun scripts/synthesis/prefix-check.ts` (note the combined hash), `bun scripts/synthesis/partition.ts --check`, `bun scripts/synthesis/units.ts check`, `bun scripts/synthesis/coverage.ts --quiet`, `bun scripts/synthesis/glossary-lint.ts`, `bun scripts/synthesis/memo.ts audit | tail -1`.
...
scripts/
  synthesis/               ← project tooling for this synthesis (pure Bun); NOT plugin content
    ...
    partition.ts           ← work units from the manifests under both caps; persists manifest/units.md (§6.5)
    units.ts               ← the unit status table docs/plan/units.md: init / status / pending / show / mark / sync / check (§7)
