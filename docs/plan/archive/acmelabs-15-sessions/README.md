> Archived 2026-08-31 (ADR-003): the `docs/plan/` README of acmelabs-15/sessions as it stood at its last commit `e0a7b2e`, kept with its two plans under their own numbers. This repo's plan of record is `../../PLAN-001-record-and-rehydration-split.md`.
...
The `/sessions:session` skill (the `sessions` plugin) writes those status lines and ticks; progress is read
...
| [PLAN-002-record-and-rehydration-split.md](PLAN-002-record-and-rehydration-split.md) | ADR-024: the record with three acts (start · log · close) and one status; rehydration moved to `/plan`; three commands; the docs sweep; the move to `~/.claude/skills` last | superseded — the copy of record is PLAN-001 in acmelabs-15/brain |
