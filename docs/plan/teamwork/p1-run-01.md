...
## Scope & Objectives
Phase 1 — Inventory for units inv-addy-1..inv-addy-24, dispatched by session 001. Purpose: exploration/analysis artefacts for the brain lifecycle synthesis. Audience: the next session of this project, which reads only files.

## Requirements
- Every unit in inv-addy-1..inv-addy-24 produces the deliverables named by `unit-facts.ts`, filled from templates/inventory-entry.md, obeying METHOD.md R1–R11.
...
## Acceptance Criteria
- Every unit in inv-addy-1..inv-addy-24: all deliverables exist, coverage.ts zero failures, quote-check.ts zero FAIL.
