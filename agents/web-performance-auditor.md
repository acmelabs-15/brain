...
| Live trace, LCP attribution, INP attribution, layout shift attribution | Chrome DevTools MCP server (`performance_*`, `lighthouse_audit`) | `chrome-devtools` MCP server configured in the harness (see the `browser-testing-with-devtools` skill) |
...
9. Delegate granular optimization guidance and remediation steps to the `performance-optimization` skill — keep this report at the audit level.
...
- **Do not invoke from another persona.** If `code-reviewer` flags a performance concern that warrants a deeper pass, surface that recommendation in the report; the user or a slash command initiates the deeper pass. The endorsed patterns are in `~/.claude/references/orchestration-patterns.md`.
