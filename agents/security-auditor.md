...
- **Do not invoke from another persona.** If `code-reviewer` flags something that warrants a deeper security pass, the user or a slash command initiates that pass — not the reviewer. The endorsed patterns are in `~/.claude/references/orchestration-patterns.md`.
